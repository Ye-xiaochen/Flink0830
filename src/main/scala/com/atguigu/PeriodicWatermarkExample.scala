...
object PeriodicWatermarkExample{
  def main(args: Array[String]): Unit = {
    ...
    val stream = env
      ...
      // 如果提前知道数据的时间是单调递增的， 也就是没有乱序存在， 可以用一下方法抽取时间戳
  ...
  class MyWindow extends ProcessWindowFunction[SensorReading, String, String, TimeWindow]{
    override def process(key: String, context: Context, elements: Iterable[SensorReading], out: Collector[String]): Unit = {
      out.collect("在传感器 " + key + " 中，窗口内共有： " + elements.size.toString + " 个熟读 " )
    }
  }
  ...
  class PeriodicAssigner extends AssignerWithPeriodicWatermarks[SensorReading] {
    // 最大延迟时间是 1 秒钟
    val bound : Long = 1000
    // 如果最大延迟时间是 0 ，说明没有存在乱序
    // val bound : Long = 0L
    // 用来保存观察到的最大事件时间戳
    var maxTs : Long = Long.MinValue

    // 这个函数什么时调用呢？ 系统插入水位线的时候， 默认 200ms 插入一次
