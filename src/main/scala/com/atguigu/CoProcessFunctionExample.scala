...
object CoProcessFunctionExample{
  def main(args: Array[String]): Unit = {
    ...
    val filterSwitches = env.fromCollection(Seq(
      ("sensor_2", 10 * 1000L)  // id 为2 的传感器读数放行 10s
    ))
  ...
  class ReadingFilter extends CoProcessFunction[SensorReading,(String, Long),SensorReading]{
    // 传送数据的开关
    // getRuntimeContext.getState首先去查找当前 key 所对应的状态中有没有名字叫做 filterSwitch的状态变量
    // 没有的话，就初始化一个
    ...
    // 来自第二条流的时间只有一条， 所有 processElement2 只会被调用一次
    override def processElement2(value: (String, Long),
      ...
      // 当碰到来自第二条流的事件，就可以把开关设置为 true，本程序打开的是 sensor_2 传感器
